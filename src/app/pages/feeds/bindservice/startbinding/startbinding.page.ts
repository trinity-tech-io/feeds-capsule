...
export class StartbindingPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    this.events.subscribe(
      ...
      (ownerDeclaredData: FeedsEvent.OwnerDeclareData) => {
        ...
        switch (phase) {
          ...
          case 'credential_issued':
            this.zone.run(() => {
              this.feedService.restoreBindingServerCache(
                this.did,
                nodeId,
                () => {
    ...
    this.events.subscribe(FeedsEvent.PublishType.issue_credential, () => {
      this.zone.run(() => {
        this.popover = this.popup.showalertdialog(
          this,
          'common.bindingCompleted',
          'IssuecredentialPage.des',
    ...
    this.events.subscribe(
      ...
      (resolveDidSucessData: FeedsEvent.ResolveDidSucessData) => {
        this.zone.run(() => {
          ...
          this.native.navigateForward(
            ['/bindservice/issuecredential', nodeId, did],
            {
              replaceUrl: true,
            },
          );
  ...
  initTitle() {
    this.titleBarService.setTitle(
      this.titleBar,
      this.translate.instant('StartbindingPage.title'),
    );
  ...
  confirm() {
    ...
    this.native
      ...
      .then(() => {
        this.feedService.startDeclareOwner(
          this.nodeId,
          this.carrierAddress,
          this.nonce,
        );
