...
export class MigrateDataService {
  ...
  //Inner method
  private getChannelData(): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const memo = { callbackMethod: FeedsData.CallbackMethod.SyncFeedsServiceData };
        await this.feedsServiceApi.getChannels(this.bindServer.nodeId, Communication.field.id, 0, 0, 0, memo);
        resolve('FINISH');
      } catch (error) {
  ...
  private handlePostData(): Promise<FeedsData.Channels> {
    return new Promise((resolve, reject) => {
      this.events.subscribe(FeedsEvent.PublishType.migrateDataToHive, (result) => {
        console.log('migrateDataToHive--result--', result);
        this.events.unsubscribe(FeedsEvent.PublishType.migrateDataToHive);
  ...
  */
  ...
  //Inner method
  private savePostDataToHive(channelId: string): Promise<string> {
    return new Promise((resolve, reject) => {
      // //遍历
      // for (let index = 0; index < array.length; index++) {
      //   const element = array[index];
      //   //发送数据到Hive
      //   this.hiveVaultController.publishPost();
      // }
    });
  }
