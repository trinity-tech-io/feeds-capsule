...
export class FeedsServiceApi {
  ...
  // createChannel(nodeId: string, name: string, introduction: string, avatar: any,): Promise<string> {
  //   return new Promise(async (resolve, reject) => {
  //     if (!(await this.hasAccessToken(nodeId))) {
  //       resolve('FINISH');
  //       return;
  //     }
  ...
  //
  getMyChannels(nodeId: string, field: Communication.field, upper_bound: number, lower_bound: number, max_counts: number) {
    if (!this.hasAccessToken(nodeId)) return;
    let accessToken: FeedsData.AccessToken =
      this.dataHelper.getAccessToken(nodeId) || null;
  ...
  getChannels(nodeId: string, field: Communication.field, upper_bound: number, lower_bound: number, max_counts: number, memo: any = ''): Promise<string> {
    if (!this.hasAccessToken(nodeId)) return;
    let accessToken: FeedsData.AccessToken =
      this.dataHelper.getAccessToken(nodeId) || null;
  ...
  //   const localSignInData =  this.dataHelper.getSigninData();
  ...
  ////
  hasAccessToken(nodeId: string): Promise<boolean> {
    return new Promise(async (resolve, reject) => {
      try {
        let accessToken = this.dataHelper.getAccessToken(nodeId) || null;
        if (this.checkExp(accessToken)) {
          await this.signinChallengeRequest(nodeId, true);
          resolve(false);
        } else {
          resolve(true);
        }
  ...
  signinChallengeRequest(nodeId: string, requiredCredential: boolean): Promise<string> {
    return new Promise(async (resolve, reject) => {
      Logger.log(TAG, 'Start signin server, nodeId is', nodeId);
      const localSignInData = await this.dataHelper.getSigninData();
